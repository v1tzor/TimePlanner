 ...
 */
@Immutable
internal data class TemplatesCategoryColors(
    val accent: Color,
    val container: Color,
)

@Composable
internal fun fetchTemplatesCategoryColors(categoryId: Long): TemplatesCategoryColors {
    val surface = MaterialTheme.colorScheme.surface
    val accent = CategoryColorsDefaults.fetchColor(categoryId)
    val container = accent
        .copy(alpha = if (surface.luminance() < 0.5f) 0.32f else 0.24f)
        .compositeOver(surface)

    return TemplatesCategoryColors(
        accent = accent,
        container = container,
    )
}
