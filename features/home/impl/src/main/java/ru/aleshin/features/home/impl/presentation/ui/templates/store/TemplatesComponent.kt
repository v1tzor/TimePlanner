 ...
 */
internal abstract class TemplatesComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: TemplatesComposeStore

    class Default(
        storeFactory: TemplatesComposeStore.Factory,
        componentContext: ComponentContext,
    ) : TemplatesComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "TEMPLATES_STORE_KEY"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = TemplatesState(),
            stateSerializer = TemplatesState.serializer(),
