...
@Composable
internal fun MainCategoryCreatorDialog(
    ...
    val textRange = TextRange(if (isEditMode) initCategoryName.length else 0)
    var mainCategoryNameValue by remember {
        mutableStateOf(TextFieldValue(text = initCategoryName, selection = textRange))
    }
    AlertDialog(onDismissRequest = onDismiss) {
        Surface(
            ...
            Column {
                MainCategoryCreatorDialogHeader()
                ...
                Row(
                    ...
                    val value = mainCategoryNameValue
                    CategoryTextMonogram(
                        text = value.text.firstOrNull()?.toString() ?: "-",
                        textColor = MaterialTheme.colorScheme.primary,
                        backgroundColor = MaterialTheme.colorScheme.primaryContainer,
                    )
                    CategoryCreatorDialogField(
                        modifier = Modifier.fillMaxWidth(),
                        subCategoryNameValue = mainCategoryNameValue,
                        isError = isError,
...
@Composable
internal fun MainCategoryCreatorDialogHeader(modifier: Modifier = Modifier) {
    Column(
