 ...
 */
internal abstract class CategoriesComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: CategoriesComposeStore

    class Default(
        storeFactory: CategoriesComposeStore.Factory,
        inputData: CategoriesInput,
        componentContext: ComponentContext,
        outputConsumer: OutputConsumer<CategoriesOutput>,
    ) : CategoriesComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "CATEGORIES_STORE_KEY"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            input = inputData,
            defaultState = CategoriesState(),
            stateSerializer = CategoriesState.serializer(),
