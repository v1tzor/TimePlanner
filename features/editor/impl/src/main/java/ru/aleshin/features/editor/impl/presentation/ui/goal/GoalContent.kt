...
import ru.aleshin.features.editor.impl.presentation.ui.goal.views.GoalEditorTopAppBar
...
internal fun GoalContent(
    ...
    Scaffold(
        ...
        topBar = {
            GoalEditorTopAppBar(
                ...
                isEditing = state.editModel?.id?.let { id -> id != 0L } == true,
                onBackClick = { store.dispatchEvent(GoalEvent.PressBack) },
            )
        },
        snackbarHost = {
    ...
    store.handleEffects { effect ->
        when (effect) {
            is GoalEffect.ShowError -> snackbarHostState.showSnackbar(
