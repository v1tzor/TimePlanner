...
internal fun GoalScopeSection(
    ...
    val scopeItems = arrayOf(
        GoalSegmentedItemUi(GoalScopeType.ALL, strings.allTasksTitle),
        GoalSegmentedItemUi(GoalScopeType.MAIN_CATEGORY, strings.categoryScopeTitle),
        GoalSegmentedItemUi(GoalScopeType.SUB_CATEGORY, strings.subCategoryScopeTitle),
    )
    ...
    Column(
        ...
        if (goal.scopeType != GoalScopeType.ALL) {
            MainCategoryChooser(
        ...
        if (goal.scopeType == GoalScopeType.SUB_CATEGORY && goal.mainCategory != null) {
            val subCategories = categories
                .firstOrNull { details -> details.mainCategory.id == goal.mainCategory.id }
                ?.subCategories
                .orEmpty()
            SubCategoryChooser(
                modifier = Modifier.fillMaxWidth(),
                isError = isScopeError && goal.subCategory == null,
                mainCategory = goal.mainCategory,
