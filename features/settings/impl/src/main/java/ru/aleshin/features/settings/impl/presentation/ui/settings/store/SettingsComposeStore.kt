 ...
 */
package ru.aleshin.features.settings.impl.presentation.ui.settings.store
...
internal class SettingsComposeStore @Inject constructor(
    ...
    override suspend fun WorkScope<SettingsState, SettingsAction, SettingsEffect, SettingsOutput>.handleEvent(
        ...
        when (event) {
            ...
            is SettingsEvent.ChangedThemeSettings -> with(event) {
                launchBackgroundWork(BackgroundKey.SETTINGS_ACTION) {
                    val command = SettingsWorkCommand.UpdateThemeSettings(themeSettings)
                    settingsWorkProcessor.work(command).collectAndHandleWork()
                }
            }
            is SettingsEvent.ChangedTasksSettings -> with(event) {
                launchBackgroundWork(BackgroundKey.SETTINGS_ACTION) {
                    val command = SettingsWorkCommand.UpdateTasksSettings(tasksSettings)
                    settingsWorkProcessor.work(command).collectAndHandleWork()
                }
            }
