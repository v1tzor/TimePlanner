 ...
 */
internal interface OverviewErrorHandler : ErrorHandler<OverviewFailures> {

    class Base @Inject constructor() : OverviewErrorHandler {
        override fun handle(throwable: Throwable) = when (throwable) {
            else -> OverviewFailures.OtherError(throwable)
        }
    }
}
