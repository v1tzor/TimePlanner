 ...
 */
data class ScheduleDetailsEntity(
    @Embedded
    ...
    val timeTasks: List<TimeTaskDetailsEntity>,
    @Relation(
        parentColumn = "date",
        entityColumn = "next_schedule_date",
        entity = TimeTaskEntity::class,
    )
    val overlayTimeTasks: List<TimeTaskDetailsEntity>,
) {
    fun <T> map(mapper: Mapper<ScheduleDetailsEntity, T>) = mapper.map(this)
}
